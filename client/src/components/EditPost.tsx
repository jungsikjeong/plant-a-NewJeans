...
import { api } from '../api';
import { fetchGetPost } from '../store/postSlice';
...
const EditPost = () => {
  ...
  const [deletedImages, setDeletedImages] = useState<any[]>([]); // 기존 이미지 삭제
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      const res = await api.post(`/posts/${id}`, formData, config);
      if (res.status === 200) {
        dispatch(clearPosts());
  ...
  useEffect(() => {
    dispatch(fetchGetPost(id as string));
  }, []);
  ...
  useEffect(() => {
    if (post) {
      ...
      post?.image?.forEach((img, index) => {
        if (img) {
          data = { index: index, url: img };
          dispatch(setImages(data));
          // setImageFile((prev: any) => [...prev, img]);
        }
      });
    }
  }, [post]);
